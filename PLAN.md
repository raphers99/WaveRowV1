...
- **[#15] Optimize Database Queries:** Replace `select('*')` with specific column selections in high-traffic queries. (DONE)
- **[#14] Standardize Supabase Client:** Use the shared `createClient()` wrapper everywhere. (DONE)
...
- **[New] Establish a Consistent Mutation Architecture:** Standardize on server actions/route handlers with shared validation.
