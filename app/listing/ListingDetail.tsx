...
export function ListingDetail({ listing, profile }: { listing: Listing; profile: ListerProfile | null }) {
  ...
  return (
    <div style={{ paddingTop: 'calc(56px + env(safe-area-inset-top))', minHeight: '100dvh', background: '#F9F9F9' }}>
      <style>{`
        /* Images span full content width above the two-column layout */
        .ld-wrap { max-width: 1160px; margin: 0 auto; padding: 16px 16px calc(80px + env(safe-area-inset-bottom)); }

        /* Two-column: stacked on mobile, side-by-side on desktop */
        .ld-cols { display: flex; flex-direction: column; gap: 20px; margin-top: 20px; }
        .ld-sidebar { display: none; }
        .ld-mobile-cta { display: block; margin-top: 4px; }

        @media (min-width: 1024px) {
          .ld-wrap { padding: 28px 40px 60px; }
          .ld-cols { flex-direction: row; align-items: flex-start; gap: 32px; margin-top: 28px; }
          .ld-left { flex: 0 0 62%; min-width: 0; }
          .ld-sidebar { display: block; flex: 1; min-width: 240px; }
        ...
        .ld-hero-grid img { width: 100%; height: 100%; object-fit: cover; display: block; }
        .ld-hero-right { display: flex; flex-direction: column; gap: 6px; }
        .ld-hero-right > div { flex: 1; overflow: hidden; }
      ...
      <div className="ld-wrap">
        ...
        {/* ── DESKTOP image: hero grid spanning full width ── */}
        ...
        {/* ── TWO-COLUMN LAYOUT (below images on all sizes) ── */}
        <div className="ld-cols">
          ...
          <div className="ld-left">
            <motion.div variants={fadeUp} initial="hidden" animate="visible" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {/* Mobile-only CTA */}
              <div className="ld-mobile-cta">{ctaButtons}</div>
          ...
          <div className="ld-sidebar">
            <div style={{ position: 'sticky', top: 'calc(72px + env(safe-area-inset-top))', background: 'white', borderRadius: 16, padding: '24px', boxShadow: '0 2px 20px rgba(0,0,0,0.08)', border: '1px solid rgba(0,0,0,0.06)' }}>
