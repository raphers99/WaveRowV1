...
function EditListingForm() {
  ...
  useEffect(() => {
    ...
    sb.auth.getSession().then(({ data }) => {
      ...
      sb.from('listings').select('*').eq('id', listingId).single().then(({ data: l, error }) => {
        if (error || !l || l.user_id !== data.session?.user.id) {
          router.replace('/dashboard')
          return
        }
        ...
        setForm(f => ({
          ...f,
          type: l.type, beds: l.beds, baths: l.baths, sqft: l.sqft ?? '', rent: String(l.rent),
          deposit: l.deposit ? String(l.deposit) : '', address: l.address,
          description: l.description || '', furnished: l.furnished, pets: l.pets,
          utilities: l.utilities, is_sublease: l.is_sublease, amenities: l.amenities || [],
          available_from: l.available_from || '', available_to: l.available_to || '', status: l.status
        }))

        // Optional sublet loading omitted for brevity unless needed deeply
      })
    })
  }, [listingId, router])
    ...
    try {
      ...
      const { error: updateErr } = await supabase
        ...
        .update({
          ...
          photos: [...photoUrls.filter(u => !u.startsWith('blob:')), ...uploadedUrls],
    ...
    return (
      <div style={{ paddingTop: 'calc(56px + env(safe-area-inset-top))', minHeight: '100dvh', background: 'var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <motion.div variants={fadeUp} initial="hidden" animate="visible" style={{ textAlign: 'center', maxWidth: 320 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-playfair)', fontSize: 26, fontWeight: 800, color: 'var(--text-primary)', margin: '0 0 10px' }}>Listing updated!</h1>
          <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: 14, color: 'var(--text-muted)', margin: '0 0 28px', lineHeight: 1.6 }}>
            Your changes are now live on WaveRow.
          </p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <motion.button
              whileTap={{ scale: 0.98 }}
              onClick={() => window.location.href = `/listing?id=${publishedId}`}
  ...
  return (
    <div style={{ paddingTop: 'calc(56px + env(safe-area-inset-top))', paddingBottom: 160, minHeight: '100dvh', background: 'var(--surface)' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: '12px 16px', paddingBottom: 'calc(12px + env(safe-area-inset-bottom))', background: 'rgba(242,242,247,0.95)', backdropFilter: 'blur(12px)', borderTop: '0.5px solid rgba(0,103,71,0.08)', zIndex: 40 }}>
        ...
        >
          {loading ? 'Saving...' : step === STEPS.length - 1 ? 'Save Changes' : 'Continue'}
        </motion.button>
      </div>
    </div>
  )
